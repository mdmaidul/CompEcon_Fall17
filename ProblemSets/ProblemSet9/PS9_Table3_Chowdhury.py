# -*- coding: utf-8 -*-
"""
Created on Tue Dec 12 22:46:17 2017
...
theta = np.array((alpha, rho, gamma, sigma_z, phi0))  # guess for initial theta
...
def z_fun(theta):
    ...
    '''    
    alpha, rho, gamma, sigma_z, phi0 = theta
...
## Function for VFI loop including necessary inequality condition
@numba.jit
...
z_discrete = fn.sim_markov(z, np.transpose(Pi), num_draws)
...
for i in range(n):
    z_new[i] = fn.sim_markov(z, np.transpose(Pi), num_draws)  #correct z : this is z for every firm
...
def moments(theta):
    ...
    '''This creates and runs the moments'''
    ...
    mu_s = np.array((max(reg_coef[1]), max(reg_coef[2]), sc_invest_nextk, sd_profit_nextk, q_bar, max(reg_coef[0])))    
...
def Qfunc2(theta):
    alpha, rho, gamma, sigma_z, phi0 = theta
    # Values assigned from panel b (row 2) (costly)
    a1 = 0.0388
    a2=0.2358
    sc=0.0158
    std=0.2614
...
bnds=((0,0.7),(0,0.2),(0,0.1),(0,1),(0,0.001))


## Applying golbal minimizer, differential_evolution

theta_hat=differential_evolution(Qfunc2,bnds)
 #coefficients at table 3
print(theta_hat.x)
print(theta_hat.message)

# Note: Order of alpha, rho, gamma, sigma, and phi are maintained when reported
# in the pdf.
...
def Qfunc3(theta_new):
    alpha, rho, gamma, sigma_z, phi0 = theta_new
    # Values assigned from panel b (row 2) (costly)
    a1 = 0.0388
    a2=0.2358
    sc=0.0158
